...
<template>
  <view
    class="shanggui"
    :class="{ mpWeixin: ISmp() }"
    :style="{ paddingTop: MBInfo().top + 10  + 'px' }"
  >
    <view class="top_tabs">
    ...
    <div :style="{ paddingTop: MBInfo().top + 10 + 'px' }" class="shanggui_con">
      <view class="tabsOne flex_r">
        <view
          class="tab_item"
          :class="{ active: s == active_m }"
          @click="onTab1(i, s)"
          v-for="(i, s) in navbar"
          :key="s"
          >{{ i.name }}
      ...
      <view class="flex_r flex_jb">
        <view v-if="active_m == 0 || active_m == 1" class="tabs_two flex_r">
          <view
            class="tab_item"
            :class="{ active: i.type == active }"
            @click="ontab2(i, s)"
            v-for="(i, s) in navbar2"
            :key="s"
            >{{ i.name }}</view
          >
        </view>
        <view v-else class="tabs_two flex_r">
          <view
            class="tab_item"
            :class="{ active: i.type == active1 }"
            @click="ontab3(i, s)"
            v-for="(i, s) in navbar3"
            :key="s"
            >{{ i.name }}</view
          >
        </view>
        ...
        <view
          @click="goto('/pages/product/exchangeRecords')"
          v-if="active_m == 2"
        ...
        <view
          v-else-if="active_m == 1"
          @click="goto('/pages/common/rulepop', { val: 'TreasureChest' })"
          class="rules BZ"
        >
        </view>
        <view
          v-else
          @click="goto('/pages/common/rulepop', { val: 'VirtualRedeem' })"
          class="rules exchange"
        >
        </view>
      </view>
      <view class="p_lists">
        <view class="tab flex_r flex_ac flex_jb">
          <view class="flex_r flex_ac">
            <view
              class="btn"
              :class="{ active: data.hasOwnProperty('order_by_create_time') }"
              @click="onBtn(0)"
              >时间
              <image
                :src="
                  data.hasOwnProperty('order_by_create_time')
                    ? 'https://img.shinemang.com/gachaStatic/chaogui/up.png'
                    : 'https://img.shinemang.com/gachaStatic/chaogui/down.png'
                "
                mode="scaleToFill"
              />
            </view>
            <view
              class="btn"
              :class="{ active: data.hasOwnProperty('order_by_inner_price') }"
              @click="onBtn(1)"
              >价值
              <image
                :src="
                  data.hasOwnProperty('order_by_inner_price')
                    ? 'https://img.shinemang.com/gachaStatic/chaogui/up.png'
                    : 'https://img.shinemang.com/gachaStatic/chaogui/down.png'
                "
                mode="scaleToFill"
        ...
        <scroll-view
          ...
          v-if="cabinetData && cabinetData.length"
          class="ListScroll"
          :lower-threshold="400"
          :scroll-y="true"
        >
          <view class="lists_box">
            <view v-if="active_m == 0" class="lists">
              ...
              >
                <view
                  class="donation"
                  v-if="item.themeType == 'ItemThemeType_Donation'"
                >
                </view>
                <view class="item_img_box">
                  ...
                  >
                    <img
                      v-if="item.isTreasureChest"
                      src="https://img.shinemang.com/gachaStatic/tag_宝箱.png"
                      alt=""
                    />
                    <view
                      :style="{
                        backgroundImage: `url(${
                          item.item.saleType == 1
                            ? 'https://img.shinemang.com/gachaStatic/chaogui/xianhuo.png'
                            : 'https://img.shinemang.com/gachaStatic/chaogui/yushou.png'
                        })`,
                      }"
                      class="item_txt1"
                    ></view>
            ...
            <view v-else class="lists">
              ...
              >
               <img
                      class="expired"
                      src="https://img.shinemang.com/gachaStatic/my/expired.png"
                      v-if="active1 == 5"
                      alt=""
                    />
                <!-- <view class="count">
      ...
      <view v-if="active_m == 0" class="activityBtn">
        <view
          v-if="
            userInfo.featureConfig &&
            (userInfo.featureConfig.decomposed == 'FeatureFlag_Enable' ||
              userInfo.featureConfig.decomposed == 'FeatureFlag_AdminOpen')
          "
          @click="onGoRecycle()"
          class="btn fs"
        ></view>
        <view
          v-if="
            userInfo.featureConfig &&
            (userInfo.featureConfig.donation == 'FeatureFlag_Enable' ||
              userInfo.featureConfig.donation == 'FeatureFlag_AdminOpen')
          "
          @click="toSendOther"
          class="btn zz"
        ></view>
      ...
      <view v-if="active_m == 0" class="foot_btn">
        <view
          v-show="userInfo.showMarket"
          @click="goto('/pages/transaction/index')"
          class="selectToExchange"
        ></view>
        <view
          @click="goto('/pages/shipments/selectGoods')"
          class="selectToSend"
        ></view>
    ...
    <u-popup
      mode="center"
      :show="confirmSendOthers"
      @close="confirmSendOthers = false"
      :closeable="true"
      round="20"
      bgColor="#fff"
    >
      <div class="send_con">
        ...
        <scroll-view class="sendList" scroll-y>
          <view class="list">
            <view
              v-for="value in sendInfo.info"
              :key="value.itemId"
              class="item"
            >
    ...
    >
      <div v-if="confirmExchangeDetail" class="confirmSureModal">
        ...
        <div class="tips">是否兑换{{ confirmExchangeDetail.item.name }} x1</div>

        <div class="actionBtns">
          <div @click="confirmExchangeModal = false" class="btn cancel">
            取消
          </div>
          <div @click="confirmExchange" class="btn confirm">确认</div>
        </div>
      </div>
    </u-popup>
    <u-popup
      mode="center"
      :show="exchangeSucessModal"
    ...
    >
      <div class="confirmSureModal">
        <div v-if="confirmExchangeDetail" class="title">
          {{ confirmExchangeDetail.item.name }}兑换码
        </div>
        ...
        <div class="actionBtns">
          <div @click="copy(code)" class="btn confirm">一键复制</div>
        </div>
      </div>
    </u-popup>
  </view>
  ...
  methods: {
    confirmExchange() {
      post("v1/cabinet/item/virtual/redeem", {
        ...
        if (!res.code) {
          this.code = res.item;
          this.exchangeSucessModal = true;
          this.confirmExchangeModal = false;
          uni.$u.toast("兑换成功");
          this.onClickResult();
        } else {
    ...
    qyExchange(item) {
      ...
      this.confirmExchangeDetail = item;
      this.confirmExchangeModal = true;
    },
    onClickResult() {
      this.pageda.page = 1;
      this.loadDetail(1);
    ...
    ontab2(item, index) {
      ...
      this.firstCondition = item.type;
      this.loadDetail(1);
    },
    ontab3(item, index) {
      this.pageda.page = 1;
      this.active1 = item.type;
      this.firstCondition = 0;
      this.loadDetail(1);
    },
    loadDetail(page) {
      ...
      post("v1/cabinet/stock/list", {
        ...
        if (!res.code) {
          ...
          if (page == 1) {
            ...
            this.cabinetData = [...this.cabinetData, ...res.cabinetStocks];
          }
    ...
    onBtn(num) {
      ...
      (this.pageda = { page: 1, page_size: 40, total: 40 }), this.loadDetail(1);
    },
    onGoRecycle() {
      this.selectType = 2;
    ...
    SelectIds(ids, infos) {
      ...
      post("v1/cabinet/decompose/cal-obtained", { item_dict: result }).then(
        (res) => {
          if (!res.code) {
            that.$showModal({
              title: "放生",
