<template>
  <view :style="{ paddingTop: MBInfo().top + 'px' }"  class="purchase">
    ...
    <div :style="{ height: conHeight }" class="selectGoods_con">
      ...
      <div class="formItem">
        ...
        <div class="totalB">
          <div></div>
          <div class="totalc">
            <div style="margin-right: 16rpx">共{{ number }}件 | 总计:</div>
            <div class="price">
              <div class="a">￥</div>
    ...
    <div class="foot_btn flex_r">
      ...
      <x-btn
        ...
        v-if="number == 0"
      />
      <x-btn class="confirm" txt="提交订单" @click="onClickDraw" v-else />
    </div>
    <!-- 留言 -->
    <remark
          ...
          if (window.mgtv) {
            mgtv.requestPaymentGameItem({
              ...
              success: (eq) => {
                ...
                let url =
                  that.saleType == 1
                    ? "/pages/my/releaseRecord"
                    : "/pages/index/index";
                if (that.saleType == 2) uni.setStorageSync("currentChange", 2);
                setTimeout(() => {
                  uni.reLaunch({
                    url: url,
                  }); // 关闭当前页面跳转到发货记录页
