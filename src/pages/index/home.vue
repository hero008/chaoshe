...
<template>
  ...
  >
      ...
      >
        ...
        >
          <view class="l_ico flex_jc flex_ac">
            ...
            <!-- v-if="mail !== '0' && mail" -->
            <view v-if="mail !== '0' && mail" class="notice_num">{{
              mail > 99 ? "99+" : mail
            }}</view>
            <!-- <view>消息中心</view> -->
          </view>
    ...
    <scroll-view
      :style="{
        height: `calc(100vh - ${MBInfo().height}px - ${
          MBInfo().top
        }px - 28rpx)`,
      }"
    ...
    >
      ...
      <view class="ad-entry flex_r flex_jb">
        <scroll-view
          class="ad_entry_scrollView"
          style="white-space: nowrap; height: 280rpx"
          scroll-x
        >
          <img
            @click.stop="toGacha(index)"
            v-for="(value, index) in gachaList"
            :key="value.url"
            :src="value.url"
            alt=""
          />
...
export default {
  ...
  methods: {
    ...
    ...mapActions(["asyncUpdateInfo", "asyncUpBalance", "asyncUpOffsetInfo"]),
    ...
    async getBannerList() {
      ...
      a?.activities?.map((i) => {
        ...
        if (i.key == "ActivityKey_RandomDiscount")
          this.RandomDiscount(Object.keys(i).length ? i.open : false);
      });
      //    }
    ...
    tn(index) {
      ...
      // 产品类型路由处理函数
      const productRoutes = {
        1: () => id !== 0 && this.goto("/pages/product/yifanshang", { id }),
        2: () => {
          if (id === 0) return;
            ...
            if (newVer > Number(toVer)) {
              if (newVer > Number(popupVersion)) {
                ...
                uni.navigateTo({
                  url:
                    "./upgrade?info=" +
                    encodeURIComponent(JSON.stringify(gres)),
                });
              }
            }
          },
